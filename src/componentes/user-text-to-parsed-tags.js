...
export const userTextToParsedUI = (text, firstLineIsTitle, fullTags) => {
    ...
    if( firstLineIsTitle )
    {
        ...
        title = <Typography component="div" variant="h4" gutterBottom><strong>{ lines.shift() }</strong></Typography>;
        text = lines.join("\n");
    }
...
const Link = ({ url })=>{
    ...
    return <a href={ url} target="_blank"><span style={{ wordBreak:"break-word" }}> { url}  </span></a> ;
}
...
export const parsedTags2render = (tags, trimTexts) => tags.map( (tag,i) => {
    ...
    switch( tag.type )
    {
        ...
        case TAGTYPE.IG: //<script async src="//www.instagram.com/embed.js"></script>
            ...
            //element = <Block><IGThumbnail id={tag.ig}/></Block>
            break;
...
//<Typography component="span" style={{fontFamily:"monospace",whiteSpace:"pre-wrap", wordBreak:"normal", wordWrap:"break-word"}}>{ tag.text }</Typography>}
    return <div key={i} style={{display:"inline" }}>
                { element? element : <UserTypedText text={ trimTexts? tag.text.trim() : tag.text } style={{fontSize: "1.3em"}}/> } 
            </div> ;
