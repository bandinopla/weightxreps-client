...
const httpLink = createUploadLink({ 
  ...
  const errorLink = onError(({ operation, graphQLErrors, networkError, response, forward }) => {
    

    //console.log( { networkError, graphQLErrors, response } );//GetSession 
...
/*
  ...
  export const DBProvider =  props => (<ApolloProvider client={client}>{ props.children }</ApolloProvider>);
  ...
  export const parseError = error => {
    ...
    {     
        let m = error.networkError.result?.errors[0].message || error.networkError.message; 
    ...
    if( rtrn=='NOTSOK' )
    {
      return <>Feature restricted to <strong>active supporters</strong> only. (Must have donated recently)</>;
    }
